...
export default function Header() {
  ...
  // A reusable component for the wallet button to avoid repetition
  const WalletButton = ({ isMobile = false }: { isMobile?: boolean }) => {
    if (isConnected) {
      return (
        <div className={`flex items-center gap-2 ${isMobile ? 'flex-col w-full' : ''}`}>
           ...
           <button
              onClick={() => disconnect()}
              className="px-6 py-2 border-2 border-electric-purple text-electric-purple font-semibold rounded-lg hover:bg-electric-purple hover:text-navy-dark w-full md:w-auto"
            >
              Disconnect
            </button>
        </div>
    ...
    return (
      <button 
        ...
        className="px-6 py-2 border-2 border-neon-green text-neon-green font-semibold rounded-lg glow-button hover:bg-neon-green hover:text-navy-dark w-full md:w-auto"
      >
        Connect Wallet
      </button>
    )
